...
if (isset($_POST['submit'])) {
    ...
    mysqli_query($con, "UPDATE tblcustomers SET Name='$name',Email='$email',MobileNumber='$mobilenum',Gender='$gender',Details='$details' WHERE ID='$uid'");
...
if (isset($_POST['changepass'])) {
    ...
    $chk = mysqli_query($con, "SELECT ID FROM tblcustomers WHERE ID='$uid' AND Password='$old'");
    if (mysqli_num_rows($chk) > 0) {
        mysqli_query($con, "UPDATE tblcustomers SET Password='$new' WHERE ID='$uid'");
...
$row = mysqli_fetch_array(mysqli_query($con, "SELECT * FROM tblcustomers WHERE ID='$uid'"));
